...
const ProfileTopBookingRequests = (props) => {
    const [colors] = useState(['#faab35', '#28bbfa', '#8a6ce0'])
    const [settings] = useState({
        dots: false,
        infinite: true,
        speed: 500,
        slidesToShow: props.bookingRequests.length < 2 ? props.bookingRequests.length : 2,
        slidesToScroll: 1,
        arrows: false
    })
    ...
    console.log(props.bookingRequests.length, "props.bookingRequests.length")
    const SlickNext = () => {
        ...
        <div className="text-white">
            ...
            <div className="flex justify-between items-center w-full mx-1">
                <div className="flex items-center">
                    <button className="px-3 py-2 text-sm hover:bg-gray-100 rounded text-gray-800 " onClick={() => { props.setNavStateProfileTop(0) }}>{type === 'professional' ? 'Your Upcoming Calls' : 'Confirmed bookings'}</button>
                    <button className="px-3 py-2 rounded bg-blue-500 hover:bg-blue-600 text-sm mx-2 text-white" style={{ backgroundColor: Colors.blue }}>{type === 'professional' ? 'Booking Requests' : 'Waiting for confirmation'}</button>
                </div>
            ...
            <div >
                <Slider className="w-full"{...settings} ref={Ref}>
...
export default ProfileTopBookingRequests


const BookingRequest = ({ time, prof_firstName, prof_lastName, professionalImage, bookingID, user, setNavState, arrLength, setBookingNavState, session_duration, backColor }) => {
