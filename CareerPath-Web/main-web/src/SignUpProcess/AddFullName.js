        ...
        <Formik
            ...
            onSubmit={(values) => {
                if (checked) {
                    ...
                    axios.get(process.env.REACT_APP_SERVER_PATH + `/invite_code/${values.VIP_CODE}/validate`).then((response) => {
                        ...
                        if (response.data.status) {
                            ...
                            axios.post(process.env.REACT_APP_SERVER_PATH + `/signup`, values)
                                .then((response) => {
                                    ...
                                    localStorage.setItem("signup_token", response.data.message.token)

                                    console.log(jwt_decode(response.data.message.token).id);
                                    props.setProfessionalId(jwt_decode(response.data.message.token).id)
                                }).catch(e => {
                                    setError(true)
        ...
        >
            ...
            {formik => (
                ...
                <div className="flex flex-col justify-center items-center">
                    ...
                    <p className="text-2xl font-bold mt-3 text-gray-800">Let’s start with the basics</p>
                    ...
                    <div className="w-full mt-4">
                        <Form>
                            ...
                            <TextField name="VIP_CODE" type="text" placeholder="VIP CODE" />
                            {codeError === true ? <p className="text-red-500 -mt-4">Vip code is invalid </p> : null}
                            ...
                            <button
                                ...
                                style={{ backgroundColor: checked && formik.values.VIP_CODE !== '' && formik.values.first_name !== '' && formik.values.last_name !== '' && formik.values.email !== '' && formik.values.password !== '' && formik.values.confirmPassword !== '' ? Colors.blue : Colors.gray, color: checked && formik.values.VIP_CODE !== '' && formik.values.first_name !== '' && formik.values.last_name !== '' && formik.values.email !== '' && formik.values.password !== '' && formik.values.confirmPassword !== '' ? 'white' : 'gray' }}>Next {isLoading ? <span className='inline-block ml-4'><Loader
                                    type="TailSpin"
                                    color="white"
                                    height={30}
                                    width={30}
